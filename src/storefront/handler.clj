...
(defn wrap-fetch-catalog [h storeback-config]
  (fn [req]
    (let [order                      (get-in-req-state req keypaths/order)
          ...
          {related-addon-skus :skus} (when (some #{"base"} (:service/type pdp-product))
                                       (api/fetch-v2-skus storeback-config
                                                          {:catalog/department "service"
                                                           :service/type       "addon"
                                                           :hair/family        (:hair/family (get pdp-skus (first (:selector/sku-ids pdp-product))))}))
      ...
      (h (-> req
             ...
             (assoc-in-req-state keypaths/v2-related-addons related-addon-skus)
