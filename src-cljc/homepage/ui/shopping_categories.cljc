...
(defn ^:private shopping-categories-box-molecule
  [{:shopping-categories.box/keys [id target ucare-id label alt-label white-text?]} react-key]
  (let [height 210
        width  171]
     ...
     [:div.bg-pale-purple
      (ui/aspect-ratio width height
                       [:a (apply utils/route-to target)
                        ...
                        (if label
                          (shopping-categories-label-atom label white-text?)
                          (shopping-categories-alt-label-atom alt-label white-text?))])]]))
