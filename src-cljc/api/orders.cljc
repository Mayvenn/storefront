...
(defn ^:private line-item-promo-discount
...
(defn ^:private order-promo-discount
  [promotion-code]
  (case promotion-code
    "heat"     15
    "cash5"    5
...
(defn ^:private shared-cart->waiter-order
...
(defn shared-cart->order
  [shared-cart state]
  (->> shared-cart
       (enrich-line-items-with-sku-data (get-in state storefront.keypaths/v2-skus))
       apply-promos
       shared-cart->waiter-order
       (api.orders/->order state)))

;; END SHARED CART CONVERSION
