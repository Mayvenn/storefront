...
(defn determine-site
  [app-state]
  (cond
    (= "mayvenn-classic" (compile-get-in app-state keypaths/store-experience)) :classic
    (= "aladdin" (compile-get-in app-state keypaths/store-experience))         :aladdin
    (= "shop" (compile-get-in app-state keypaths/store-slug))                  :shop))
...
(defn refresh-account [app-state]
  (let [user-id (compile-get-in app-state keypaths/user-id)
        user-token (compile-get-in app-state keypaths/user-token)]
  ...
  (let [order                (compile-get-in app-state keypaths/order)
    ...
    (when servicing-stylist-id
      (api/fetch-matched-stylist
       (compile-get-in app-state keypaths/api-cache)
...
(defn refresh-current-order [app-state]
  (let [user-id      (compile-get-in app-state keypaths/user-id)
        user-token   (compile-get-in app-state keypaths/user-token)
        stylist-id   (compile-get-in app-state keypaths/store-stylist-id)
        order-number (compile-get-in app-state keypaths/order-number)
        order-token  (compile-get-in app-state keypaths/order-token)]
    (cond
...
(defmethod effects/perform-effects events/app-start [dispatch event args _ app-state]
  (let [choices (-> (compile-get-in app-state keypaths/cookie)
  ...
  (lucky-orange/track-store-experience (compile-get-in app-state keypaths/store-experience))
  (doseq [feature (compile-get-in app-state keypaths/features)]
...
(defn redirect-from-deals-page? [app-state]
  (let [[current-nav-event current-nav-args] (compile-get-in app-state keypaths/navigation-message)
        shop-or-aladdin?                     (or (= "shop" (compile-get-in app-state keypaths/store-slug))
...
(defmethod effects/perform-effects events/escape-key-pressed [_ event args _ app-state]
  (when-let [message-to-handle (get popup-dismiss-events (compile-get-in app-state keypaths/popup))]
...
(defmethod effects/perform-effects events/ensure-sku-ids
  ...
  (let [ids-in-db   (keys (compile-get-in app-state keypaths/v2-skus))
        missing-ids (seq (set/difference (set sku-ids)
                                         (set ids-in-db)))

        api-cache (compile-get-in app-state keypaths/api-cache)
...
(defmethod effects/perform-effects events/external-redirect-welcome [_ event args _ app-state]
  (set! (.-location js/window) (compile-get-in app-state keypaths/welcome-url)))
...
(defmethod effects/perform-effects events/external-redirect-paypal-setup [_ event args _ app-state]
  (set! (.-location js/window) (compile-get-in app-state keypaths/order-cart-payments-paypal-redirect-url)))
...
(defmethod effects/perform-effects events/control-navigate [_ event {:keys [navigation-message]} _ app-state]
  ...
  (messages/handle-message events/navigation-save (-> (compile-get-in app-state keypaths/navigation-stashed-stack-item)
...
(defmethod effects/perform-effects events/browser-navigate
  ...
  (let [leaving-stack-item {:final-scroll js/window.scrollY}
        back               (first (compile-get-in app-state keypaths/navigation-undo-stack))
        forward            (first (compile-get-in app-state keypaths/navigation-redo-stack))]
...
(defn add-pending-promo-code [app-state {:keys [number token]}]
  (when-let [pending-promo-code (compile-get-in app-state keypaths/pending-promo-code)]
    (api/add-promotion-code {:shop?              (= "shop" (compile-get-in app-state keypaths/store-slug))
                             :session-id         (compile-get-in app-state keypaths/session-id)
                             :number             number
                             :token              token
...
(defmethod effects/perform-effects events/navigate
  ...
  (let [new-nav-event?     (not= (compile-get-in prev-app-state keypaths/navigation-event)
                                 (compile-get-in app-state keypaths/navigation-event))
        new-query-params?  (not= (not-empty (dissoc (compile-get-in prev-app-state keypaths/navigation-args) :query-params))
                                 (not-empty (dissoc (compile-get-in app-state keypaths/navigation-args) :query-params)))
    ...
    (messages/handle-message events/save-order {:order (compile-get-in app-state keypaths/order)})

    (cookie-jar/save-user (compile-get-in app-state keypaths/cookie)
                          (compile-get-in app-state keypaths/user))
    (refresh-account app-state)

    (when-not (or module-load? (#{:first-nav} caused-by))
      (api/get-promotions (compile-get-in app-state keypaths/api-cache)
                          (or
                           (first (compile-get-in app-state keypaths/order-promotion-codes))
                           (compile-get-in app-state keypaths/pending-promo-code))))
    ...
    (when-let [pending-promo-code (:sha query-params)]
      (cookie-jar/save-pending-promo-code
       (compile-get-in app-state keypaths/cookie)
       pending-promo-code))

    (when-let [affiliate-stylist-id (:affiliate_stylist_id query-params)]
      (cookie-jar/save-affiliate-stylist-id (compile-get-in app-state keypaths/cookie)
    ...
    (when-not module-load?
      (when (compile-get-in app-state keypaths/popup)
    ...
    (when (and (compile-get-in app-state keypaths/user-must-set-password)
...
(defmethod effects/perform-effects events/navigate-content
  ...
  (when-not (= static-content-id
               (compile-get-in app-state keypaths/static-id))
  ...
  (let [actual-album-kw (ugc/determine-look-album app-state album-keyword)]
    (cond
      (and (#{:wavy-curly-looks :straight-looks} album-keyword)
           (not= (compile-get-in app-state keypaths/store-slug) "shop")
           (not= "aladdin" (compile-get-in app-state keypaths/store-experience)))
...
(defmethod effects/perform-effects events/navigate-account [_ event args _ app-state]
  (when-not (compile-get-in app-state keypaths/user-token)
...
(defmethod effects/perform-effects events/navigate-stylist [_ event args _ app-state]
  (when (not (and (compile-get-in app-state keypaths/user-token)
                  (compile-get-in app-state keypaths/user-store-id)))
    (effects/redirect events/navigate-sign-in)))
...
(defmethod effects/perform-effects events/control-email-captured-submit [_ _ args _ app-state]
  (when (empty? (compile-get-in app-state keypaths/errors))
    (cookie-jar/save-email-capture-session (compile-get-in app-state keypaths/cookie) "opted-in")))
...
(defmethod effects/perform-effects events/api-end [_ event args previous-app-state app-state]
  (let [app-version    (compile-get-in app-state keypaths/app-version)
...
(defmethod effects/perform-effects events/navigate-cart [_ event args _ app-state]
  ...
  (api/get-states (compile-get-in app-state keypaths/api-cache))
...
(defn ensure-bucketed-for [app-state experiment]
  (let [already-bucketed? (contains? (compile-get-in app-state keypaths/experiments-bucketed) experiment)]
...
(defmethod effects/perform-effects events/navigate-checkout [_ event args _ app-state]
  (google-maps/insert)
  (let [have-cart? (compile-get-in app-state keypaths/order-number)]
    (when (not have-cart?)
...
(defn- fetch-saved-cards [app-state]
  (when-let [user-id (compile-get-in app-state keypaths/user-id)]
    (api/get-saved-cards user-id (compile-get-in app-state keypaths/user-token))))
...
(defmethod effects/perform-effects events/navigate-checkout-payment [dispatch event args _ app-state]
  (when (empty? (compile-get-in app-state keypaths/order-shipping-address))
    (effects/redirect events/navigate-checkout-address))
  (fetch-saved-cards app-state)
  (stripe/insert)
  (quadpay/insert))
...
(defmethod effects/perform-effects events/navigate-checkout-confirmation [_ event args _ app-state]
  ...
  (when (empty? (compile-get-in app-state keypaths/order-cart-payments))
...
(defmethod effects/perform-effects events/navigate-order-complete [_ event {{:keys [paypal order-token]} :query-params number :number} _ app-state]
  ...
  (let [servicing-stylist    (compile-get-in app-state adv-keypaths/adventure-servicing-stylist)
        servicing-stylist-id (compile-get-in app-state adv-keypaths/adventure-choices-selected-stylist-id)]
    (when (and servicing-stylist-id (not servicing-stylist))
      (api/fetch-matched-stylist (compile-get-in app-state keypaths/api-cache) servicing-stylist-id))))

(defmethod effects/perform-effects events/navigate-need-match-order-complete
  [_ event {{:keys [paypal]} :query-params} _ app-state]
  (let [{:keys [number token] :as order} (compile-get-in app-state keypaths/completed-order)]
    (when (not (compile-get-in app-state keypaths/user-id))
      (facebook/insert))
    (when (and number token)
      (api/get-completed-order number token))
    (when (not (compile-get-in app-state adv-keypaths/adventure-matched-stylists))
...
(defn redirect-when-signed-in [app-state]
  (when (compile-get-in app-state keypaths/user-email)
    (if (compile-get-in app-state keypaths/telligent-community-url)
...
(defmethod effects/perform-effects events/navigate-sign-out [_ _ {{:keys [telligent-url]} :query-params} _ app-state]
  (if telligent-url
    (do
      (cookie-jar/clear-telligent-session (compile-get-in app-state keypaths/cookie))
...
(defmethod effects/perform-effects events/control-sign-in-submit [_ event args _ app-state]
  (api/sign-in (compile-get-in app-state keypaths/session-id)
               (compile-get-in app-state keypaths/stringer-browser-id)
               (compile-get-in app-state keypaths/sign-in-email)
               (compile-get-in app-state keypaths/sign-in-password)
               (compile-get-in app-state keypaths/store-stylist-id)
               (compile-get-in app-state keypaths/order-number)
               (compile-get-in app-state keypaths/order-token)))

(defmethod effects/perform-effects events/control-sign-up-submit [_ event _ _ app-state]
  (let [{:keys [number token]} (or (compile-get-in app-state keypaths/order)
                                   (compile-get-in app-state keypaths/completed-order))]
    (api/sign-up (compile-get-in app-state keypaths/session-id)
                 (compile-get-in app-state keypaths/stringer-browser-id)
                 (compile-get-in app-state keypaths/sign-up-email)
                 (compile-get-in app-state keypaths/sign-up-password)
                 (compile-get-in app-state keypaths/store-stylist-id)
                 number
                 token)))
...
(defmethod effects/perform-effects events/facebook-success-sign-in [_ _ {:keys [authResponse]} _ app-state]
  (let [{:keys [number token]} (or (compile-get-in app-state keypaths/order)
                                   (compile-get-in app-state keypaths/completed-order))]
    (api/facebook-sign-in (compile-get-in app-state keypaths/session-id)
                          (compile-get-in app-state keypaths/stringer-browser-id)
                          (:userID authResponse)
...
(defmethod effects/perform-effects events/control-forgot-password-submit [_ event args _ app-state]
  (api/forgot-password (compile-get-in app-state keypaths/session-id) (compile-get-in app-state keypaths/forgot-password-email)))
...
(defmethod effects/perform-effects events/control-account-profile-submit [_ event args _ app-state]
  (when (empty? (compile-get-in app-state keypaths/errors))
    (api/update-account (compile-get-in app-state keypaths/session-id)
                        (compile-get-in app-state keypaths/user-id)
                        (compile-get-in app-state keypaths/manage-account-email)
                        (compile-get-in app-state keypaths/manage-account-password)
                        (compile-get-in app-state keypaths/user-token))))

(defmethod effects/perform-effects events/uploadcare-api-success-upload-portrait [_ _ {:keys [cdnUrl]} _ app-state]
  (let [user-id    (compile-get-in app-state keypaths/user-id)
        user-token (compile-get-in app-state keypaths/user-token)
        stylist-id (compile-get-in app-state keypaths/user-store-id)
        session-id (compile-get-in app-state keypaths/session-id)]
...
(defmethod effects/perform-effects events/control-reset-password-submit [_ event args _ app-state]
  (if (empty? (compile-get-in app-state keypaths/reset-password-password))
    (messages/handle-message events/flash-show-failure {:message "Your password cannot be blank."})
    (api/reset-password (compile-get-in app-state keypaths/session-id)
                        (compile-get-in app-state keypaths/stringer-browser-id)
                        (compile-get-in app-state keypaths/reset-password-password)
                        (compile-get-in app-state keypaths/reset-password-token)
                        (compile-get-in app-state keypaths/order-number)
                        (compile-get-in app-state keypaths/order-token)
                        (compile-get-in app-state keypaths/store-stylist-id))))

(defmethod effects/perform-effects events/facebook-success-reset [_ event facebook-response _ app-state]
  (api/facebook-reset-password (compile-get-in app-state keypaths/session-id)
                               (compile-get-in app-state keypaths/stringer-browser-id)
...
(defmethod effects/perform-effects events/control-checkout-update-addresses-submit [_ event args _ app-state]
  (let [guest-checkout? (compile-get-in app-state keypaths/checkout-as-guest)
        billing-address (compile-get-in app-state keypaths/checkout-billing-address)
        shipping-address (compile-get-in app-state keypaths/checkout-shipping-address)
        update-addresses (if guest-checkout? api/guest-update-addresses api/update-addresses)]
    (update-addresses
     (compile-get-in app-state keypaths/session-id)
     (cond-> (merge (select-keys (compile-get-in app-state keypaths/order) [:number :token])
                    {:billing-address billing-address :shipping-address shipping-address})
       guest-checkout?
       (assoc :email (compile-get-in app-state keypaths/checkout-guest-email))

       (compile-get-in app-state keypaths/checkout-bill-to-shipping-address)
...
(defmethod effects/perform-effects events/control-checkout-shipping-method-select [_ event args _ app-state]
  (api/update-shipping-method (compile-get-in app-state keypaths/session-id)
                              (merge (select-keys (compile-get-in app-state keypaths/order) [:number :token])
                                     {:shipping-method-sku (compile-get-in
                                                            app-state
    ...
    (api/update-cart-payments
     ...
     {:order        (-> order
                        ...
                        (assoc :cart-payments (merge {:stripe {:source         (:id token)
                                                               ...
                                                               :save?          (boolean (and (compile-get-in app-state keypaths/user-id)
                                                                                             (compile-get-in app-state keypaths/checkout-credit-card-save)))}}
...
(defn create-stripe-token [app-state args]
  ...
  (stripe/create-token (compile-get-in app-state keypaths/stripe-card-element)
                       (compile-get-in app-state keypaths/checkout-credit-card-name)
...
(defmethod effects/perform-effects events/order-remove-promotion [_ _ {:keys [code hide-success]} _ app-state]
  (api/remove-promotion-code
   (compile-get-in app-state keypaths/session-id)
   (compile-get-in app-state keypaths/order)
   code
...
(defmethod effects/perform-effects events/order-remove-freeinstall-line-item [_ _ _ _ app-state]
  (api/remove-freeinstall-line-item (compile-get-in app-state keypaths/session-id) (compile-get-in app-state keypaths/order)))
...
(defmethod effects/perform-effects events/control-checkout-confirmation-submit [_ event {:keys [place-order?] :as args} _ app-state]
  (let [order (compile-get-in app-state keypaths/order)]
    (if place-order?
      ...
      (api/place-order (compile-get-in app-state keypaths/session-id)
                       order
                       (cookie-jar/retrieve-utm-params (compile-get-in app-state keypaths/cookie))
  ...
  (let [servicing-stylist-id          (:servicing-stylist-id order)
        ...
        servicing-stylist-not-loaded? (and servicing-stylist-id
                                           (not=
                                            servicing-stylist-id
                                            (-> app-state
                                                (compile-get-in adv-keypaths/adventure-servicing-stylist)
                                                :stylist-id)))]
    (if incomplete-order?
      (do
        ...
        (when servicing-stylist-not-loaded?
          (api/fetch-matched-stylist (compile-get-in app-state keypaths/api-cache) servicing-stylist-id))
        (cookie-jar/save-order (compile-get-in app-state keypaths/cookie) order)
...
(defmethod effects/perform-effects events/clear-order [_ _ _ _ app-state]
  (cookie-jar/clear-order (compile-get-in app-state keypaths/cookie)))
...
(defmethod effects/perform-effects events/poll-stylist-portrait [_ event args _ app-state]
  (api/refresh-stylist-portrait (compile-get-in app-state keypaths/user-id)
                                (compile-get-in app-state keypaths/user-token)
                                (compile-get-in app-state keypaths/user-store-id)))
...
(defmethod effects/perform-effects events/api-success-stylist-account
  ...
  (when-let [became-pending? (and
                              ...
                              (= "pending" (compile-get-in app-state keypaths/stylist-portrait-status)))]
...
(defmethod effects/perform-effects events/api-success-stylist-account-portrait [_ event {:keys [updated?]} previous-app-state app-state]
  ...
  (when-let [still-pending? (= "pending"
                               (compile-get-in previous-app-state keypaths/stylist-portrait-status)
                               (compile-get-in app-state keypaths/stylist-portrait-status))]
    (messages/handle-later events/poll-stylist-portrait {} 5000)))
...
(defmethod effects/perform-effects events/order-completed
  ...
  (let [site (determine-site app-state)]
    (cookie-jar/save-completed-order (compile-get-in app-state keypaths/cookie)
                                     (compile-get-in app-state keypaths/completed-order))
    (messages/handle-message events/clear-order)
...
(defmethod effects/perform-effects events/api-success-get-order [_ event order _ app-state]
  (when-let [servicing-stylist-id (:servicing-stylist-id order)]
    (api/fetch-matched-stylist (compile-get-in app-state keypaths/api-cache)
                               servicing-stylist-id))
  (messages/handle-message events/save-order {:order order}))
...
(defmethod effects/perform-effects events/api-failure-pending-promo-code [_ event args _ app-state]
  (cookie-jar/clear-pending-promo-code (compile-get-in app-state keypaths/cookie)))
...
(defmethod effects/perform-effects events/api-failure-errors [_ event {:keys [error-code scroll-selector] :as errors} _ app-state]
  (condp = error-code
    "stripe-card-failure"         (when (= (compile-get-in app-state keypaths/navigation-event)
    ...
    "invalid-input"               (if scroll-selector
                                    ...
                                    (scroll/snap-to-top))
    "ineligible-for-free-install" (when (= (compile-get-in app-state keypaths/navigation-event)
                                           events/navigate-checkout-confirmation)
                                    (api/remove-promotion-code (compile-get-in app-state keypaths/session-id)
                                                               (compile-get-in app-state keypaths/order)
                                                               "freeinstall"
...
(defmethod effects/perform-effects events/reviews-component-mounted [_ event args _ app-state]
  (let [expected-reviews-count 2
        actual-reviews-count   (compile-get-in app-state keypaths/review-components-count)]
...
(defmethod effects/perform-effects events/control-email-captured-dismiss [_ event args _ app-state]
  (cookie-jar/save-email-capture-session (compile-get-in app-state keypaths/cookie) "dismissed"))
...
(defmethod effects/perform-effects events/sign-out [_ event args app-state-before app-state]
  (when (not= "opted-in" (popup/email-capture-session app-state))
    (cookie-jar/save-email-capture-session (compile-get-in app-state keypaths/cookie) "dismissed"))
  (messages/handle-message events/clear-order)
  (cookie-jar/clear-account (compile-get-in app-state keypaths/cookie))
  (messages/handle-message events/control-menu-collapse-all)
  (abort-pending-requests (compile-get-in app-state keypaths/api-requests))
  (if (= events/navigate-home (compile-get-in app-state keypaths/navigation-event))
  ...
  (api/sign-out (compile-get-in app-state keypaths/session-id)
                (compile-get-in app-state keypaths/stringer-browser-id)
                (compile-get-in app-state-before keypaths/user-id)
                (compile-get-in app-state-before keypaths/user-token)))

(defmethod effects/perform-effects events/navigate-voucher [_ event args app-state-before app-state]
  (api/fetch-user-stylist-service-menu (compile-get-in app-state keypaths/api-cache)
                                       {:user-id    (compile-get-in app-state keypaths/user-id)
                                        :user-token (compile-get-in app-state keypaths/user-token)
                                        :stylist-id (compile-get-in app-state keypaths/user-store-id)}))
...
(defmethod effects/perform-effects events/module-loaded [_ _ {:keys [module-name for-navigation-event]} app-state-before app-state]
  (let [already-loaded-module? (= (compile-get-in app-state-before keypaths/modules)
                                  (compile-get-in app-state keypaths/modules))
        [evt args]             (compile-get-in app-state keypaths/navigation-message)]
