...
(defn insert-tracking []
  (insert-tag-with-text
   (str "(function() {var stringer = window.stringer = window.stringer || [],methods = ['init', 'track']; function stub(method) {return function() {var args = Array.prototype.slice.call(arguments); args.unshift(method); stringer.push(args); return stringer;};} for (var i = 0; i < methods.length; i++) {var k = methods[i]; stringer[k] = stub(k);}
...
stringer.init({serverURI: '" config/byliner-uri "',sourceSite: 'storefront'});})(); ") "stringer"))
